 ...
 * [Unit-тесты](https://ru.wikipedia.org/wiki/Модульное_тестирование). Методика используемая при тестировании приложений или отдельных его частей. Наличие тестов сразу несколько преимуществ. Возможность быстро проверять работоспособность внесенных в код изменений. И способ показать спецификациию метода(-ов) класса: какие входные параметры ожидает этот метод, и что остальные компоненты системы ждут от него на выходе.
